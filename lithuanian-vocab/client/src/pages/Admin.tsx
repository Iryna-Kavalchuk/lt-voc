...
import { api, type AdminStats, type HistogramBucket, type DictionaryStats } from "../api/client";
...
function DictSection({ d }: { d: DictionaryStats }) {
  return (
    <section className="admin-section">
      <h3 className="admin-section-title">{d.dictionary}</h3>
      <StatCards counts={d.counts} accuracy={d.accuracy} />
      <Histogram buckets={d.histogram} />
    </section>
  );
}
...
export default function Admin() {
  ...
  return (
    <div className="admin-page">
      ...
      <section className="admin-section">
        <h3 className="admin-section-title">All dictionaries</h3>
        <StatCards counts={stats.counts} accuracy={stats.accuracy} />
        <Histogram buckets={stats.histogram} />
      </section>

      {stats.byDictionary.map((d) => (
        <DictSection key={d.dictionary} d={d} />
      ))}
    </div>
  );
}
